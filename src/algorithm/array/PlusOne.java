...
public class PlusOne {
    public static void main(String[] args) {
        ...
        int b[]={9,9,9,9};
        System.out.println(Arrays.toString(plusOne3(b)));
    }
    ...
    转string，计算后再转回来
     */
    public static int[] plusOne3(int a[]){
        String number= Arrays.toString(a).replace("[","").replace(",","").replace("]","").replace(" ","");
        BigInteger big=BigInteger.valueOf(Long.valueOf(number));
        big=big.add(new BigInteger("1"));
        String backString=big.toString();
        int[] backNumber=new int[backString.length()];
        for (int i = 0; i < backString.length(); i++) {
            backNumber[i]=Integer.valueOf(backString.substring(i,i+1));
        }
        return backNumber;
    }
}
